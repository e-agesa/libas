...
// Local cart for display with qty adjustments
...
const cartTotal = computed(() => displayItems.value.reduce((sum, i) => sum + (i.price * i.qty), 0));
const cartCount = computed(() => displayItems.value.reduce((sum, i) => sum + i.qty, 0));
        ...
        <header class="bg-white border-b border-gray-200 sticky top-0 z-30">
            <div class="max-w-4xl mx-auto px-4 sm:px-6">
                <div class="flex items-center justify-between h-14">
                    <Link :href="route('shop')" class="flex items-center gap-2 text-gray-600 hover:text-green-600 transition-colors">
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
            ...
            <div class="flex items-center justify-center gap-2 sm:gap-4 mb-8">
                <div class="flex items-center gap-1.5">
                    <span class="flex h-6 w-6 items-center justify-center rounded-full bg-green-600 text-white text-xs font-bold">1</span>
                    <span class="text-xs sm:text-sm font-medium text-green-700">Cart</span>
                </div>
                <div class="w-8 sm:w-12 h-px bg-green-300"></div>
                <div class="flex items-center gap-1.5">
                    <span class="flex h-6 w-6 items-center justify-center rounded-full bg-green-600 text-white text-xs font-bold">2</span>
                    <span class="text-xs sm:text-sm font-medium text-green-700">Details</span>
                </div>
            ...
            <div v-if="displayItems.length === 0" class="text-center py-16">
                ...
                <Link :href="route('shop')" class="inline-flex items-center gap-1 mt-4 rounded-lg bg-green-600 text-white px-6 py-2.5 text-sm font-medium hover:bg-green-700">
                ...
                <div class="lg:col-span-3 space-y-6">
                    ...
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                        <div class="px-4 sm:px-6 py-4 border-b border-gray-100 bg-gray-50">
                            <h2 class="text-sm font-semibold text-gray-900"><i class="pi pi-shopping-cart mr-2 text-green-600"></i>Order Items ({{ cartCount }})</h2>
                        </div>
                    ...
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                        <div class="px-4 sm:px-6 py-4 border-b border-gray-100 bg-gray-50">
                            <h2 class="text-sm font-semibold text-gray-900"><i class="pi pi-user mr-2 text-green-600"></i>Your Details</h2>
                        ...
                        <div class="p-4 sm:p-6 space-y-4">
                            <div>
                                ...
                                <input v-model="form.name" type="text" placeholder="e.g. Ali Hassan"
                                    class="w-full rounded-xl border-gray-200 text-sm focus:border-green-500 focus:ring-green-500 py-2.5" />
                                <p v-if="form.errors.name" class="text-xs text-red-500 mt-1">{{ form.errors.name }}</p>
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">Phone / WhatsApp <span class="text-red-400">*</span></label>
                                <div class="relative">
                                    <i class="pi pi-whatsapp absolute left-3 top-1/2 -translate-y-1/2 text-green-500 text-sm"></i>
                                    <input v-model="form.phone" type="tel" placeholder="+254 7XX XXX XXX"
                                        class="w-full rounded-xl border-gray-200 text-sm focus:border-green-500 focus:ring-green-500 py-2.5 pl-9" />
                                </div>
                            ...
                            <div>
                                ...
                                <textarea v-model="form.notes" rows="2" placeholder="Any special instructions..."
                                    class="w-full rounded-xl border-gray-200 text-sm focus:border-green-500 focus:ring-green-500"></textarea>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="lg:col-span-2">
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden sticky top-20">
                        <div class="px-4 sm:px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-green-50 to-emerald-50">
                            <h2 class="text-sm font-semibold text-gray-900"><i class="pi pi-receipt mr-2 text-green-600"></i>Order Summary</h2>
                        </div>
                        <div class="p-4 sm:p-6 space-y-3">
                            ...
                            <div class="border-t border-gray-100 pt-3 mt-3">
                                ...
                                <div class="flex justify-between text-sm mt-1">
                                    ...
                                    <span class="text-green-600 text-xs font-medium">To be confirmed</span>
                                </div>
                            </div>
                            <div class="border-t border-gray-200 pt-3">
                                <div class="flex justify-between">
                                    <span class="font-semibold text-gray-900">Total</span>
                                    <span class="text-xl font-bold text-green-700">{{ formatCurrency(cartTotal) }}</span>
                                </div>
                            </div>
                        </div>
